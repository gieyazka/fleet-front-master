...
class Report extends React.Component {
  ...
  componentDidMount() {
    ...
    const q_start_date = `date_gte=${moment(this.state.data.created_date)
      .add(-7, 'hours') // local-time bug
      .startOf('day')
      .add(-1, 'hours')}`;
    console.log(this.state.data.created_date);
    console.log(q_start_date);
    const q_end_date = this.state.data.delivered_date
      ? `date_lte=${moment(this.state.data.delivered_date)
          .add(-7, 'hours')
          .endOf('day')
          .add(1, 'hours')}`
      : `date_lte=${moment(this.state.data.created_date)
          .add(-7, 'hours')
          .endOf('day')
          .add(1, 'hours')}`;
  ...
  columns = [
    {
      ...
      columns: [
        ...
        },
        {
          ...
          accessor: 'status',
          Cell: (row) => <span> {row.value ? 'In' : 'Out'}</span>,
        },
        {
          Header: 'Site',
          accessor: 'site',
    ...
    },
    {
      ...
      columns: [
        ...
        },
        {
          ...
          accessor: 'dest_use',
          Cell: (row) => <span>{row.value ? '✔️' : null}</span>,
          width: 100,
          filterable: false,
        },
        {
