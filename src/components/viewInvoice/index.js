    ...
    return (
      <Grid container style={{justifyContent: 'center', margin: '10px'}}>
        ...
        <Paper style={{width: '100%', margin: 10, textAlign: 'center', display: 'contents'}}>
          ...
          <Button variant="contained" size="small" className={classes.button} onClick = {()=>{this.props.history.push('print', {id: data.id})}}>
...

          {
            ((data.issuer === localStorage.getItem('username') || localStorage.getItem('role') === 'admin')  && (data.status !== "confirm"))?(
              <Button variant="contained" size="small" className={classes.button} onClick = {()=>{this.props.history.push('editinvoice', {id: data.id})}}>
        ...
        <Typography> Order status: {' '}
          <Chip label={data.status.replace('_', ' ')} key={data.status} color="primary"/> 
          <br />
        ...
        <Grid container style={{justifyContent: 'center'}}>
              ...
              <Grid item xs={6} className={classes.box}>Amount | ราคา: {data.price}{' '} Baht</Grid>
              ...
              <Grid item xs={4} style={{padding: 20, textAlign: 'center', marginBottom: 30}} className={classes.box}>
                    Issuer's name |   
                    ผู้สั่งงาน: <br/> <b>{data.issuer}</b>
              </Grid>
              <Grid item xs={4} style={{padding: 20, textAlign: 'center', marginBottom: 30}} className={classes.box}>
                    Checked by |   
                    ผู้ตรวจงาน: <br/> {data.checked_by}
              </Grid>
            </Grid>
        </Paper>
      </Grid>
