            ...
            <Table className={classes.table}>
                    <TableBody>
                        <TableRow>
                            ...
                            <TableCell >{this.props.data.created_date}</TableCell>
                        </TableRow>
                        <TableRow>
                            <TableCell component="th" scope="row">
                                <b>Invoice</b>
                        ...
                        <TableRow>
                            ...
                            <TableCell >
                            {Object.keys(this.props.data).length !== 0 ?this.props.data.products.split("|").map(product => 
                                {   
                                    console.log(product)
                                    if (product === 'Pallet'){
                                        return <Chip label={`Pallet (${this.props.data.num_pallet})`} key={product} color="primary"/>
                                      } else if (product === 'Rack'){
                                        return <Chip label={`Rack (${this.props.data.num_rack})`} key={product} color="primary"/>
                                      } else if (product === 'Dies'){
                                        return <Chip label={`Dies (${this.props.data.num_die})`} key={product} color="primary"/>
                                      } else if (product === 'Box'){
                                        return <Chip label={`Box (${this.props.data.num_box})`} key={product} color="primary"/>
                                      } else {
